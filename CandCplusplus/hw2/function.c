...
int getLine(char *longest, char  *current){
	while((c = getchar()) != EOF){
		if(cnt <= 98){
			...
			cnt = 0;
			return 101;// if I put -1 it would exit loop in the main function so I had to not comply completely with instructions, otherwise it would only print the longest line before ones that overun the buffer.
		}else{}
		cnt++;
	}
...
void copy(char to[], char from[]){
	...
	for(i = 0; from[i] != '\0' /*i < MAX_LINE_LENGTH*/; i++){
		temp = from[i];
		to[i] = temp;
	}
	to[i] = '\0';
