...
This code is meant for lab9. it is meant to be used alongside the lex file
...
Afterwards it will print out the MIPS code into the specified file

Arguments:
   -o for file output, such as -o foo for foo.asm
   -d for debug flag
...
Changes:
added string printing and flag options
*/
...
write_stmt	: WRITE expression ';'	{if (mydebug) fprintf(stderr, "at write_stmt  \n");
            ...
            | WRITE STRING ';'      {if (mydebug) printf("found a write string with string %s\n", $2);
                                    $$ = ASTCreateNode(MYWRITE);
                                    $$->Name = $2;
                                    $$->label = genlabel();
                                    }
...
FILE *fd;
int foundit = 0;
while(i < argc){//process our argument list
  //check if we should set mydebug to 1
  if(strcmp(argv[i], "-d") == 0){
    mydebug = 1;
    i = i + 1;
  }else if (strcmp(argv[i], "-o")==0){//check for file output
    if(argc > i + 1) foundit=1;
  //we assume that the i+1 is a file prefix we want to open
...
if (foundit == 0){
    //if no file output was given then error
    printf("you have to give me a -o argument \n");
    exit(1);
}
//we want to now open a file descriptor nd use that variable to write to our open file
fd = fopen(s, "w");
if(fd == NULL){
...
if(isSyntaxError){
    ...
    if (mydebug) Display();
    if (mydebug) ASTprint(GlobalTreePointer, 0);
    
    //print data segment
    fprintf(fd, ".data\n");
    EMITSTRINGS(GlobalTreePointer, fd);
    //print strings
    fprintf(fd, "_NL:\t .asciiz\t\"\\n\"\n");
    fprintf(fd, "\n.align 2\n");
    //print global variables
    EMITGLOBALS(GlobalTreePointer, fd);
    fprintf(fd, "\n.text\n");
    fprintf(fd, "\n.globl main\n\n");
    //print the body of the code
    EMITAST(GlobalTreePointer, fd); //print out the body of MIPS code
    }
}
