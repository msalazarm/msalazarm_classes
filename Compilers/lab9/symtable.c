...
Algo) The algorithms are simple variations on linked list algorithms. one inserts into the linked list, while one deletes elements of a certain level and higher.
  a) the input is ussually the identifying imformation needed to find, insert, or deleter certain entries
  b) the output is either a pointer to the element, or the size of all that was deleted
  c) The input should be a valid entry for a symbol table with no repeats in the same level.
*/
 ...
 * pre: The symbol has not been inserted yet at this level
 * post: Will return a refrence to the inserted SymbTab
 ...
 * pre: a refrence to a symbtab
 * post: all the information about the symbtab printed out
 */
void PrintSym(struct SymbTab *s)
{
         printf("\t%s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",s->name,s->offset, s->level, s->Type, s->IsAFunc, s->mysize);
 ...
 * pre:None
 * Post: will print out all of the available symbtabs
 */
...
/*  Search for a symbol name at level or below.  We have to do multiple passes into the symbol table because we have to find
  ...
  If recur is non-zero, then we look through all of the levels, otherwise, only our level 
   ...
   Pre: what we are searching for is in the linked list
   Post: will either return the refrence to the element we are looking for or NULL
...
/* Remove all enteries that have the indicated level
   ...
   Pre:None
   Post: will delete all elements that are at the specified level
   */
